...
Route::group(['namespace' => 'Web','middleware'=>['maintenance_mode']], function () {
    Route::get('noqoody/callback','WebController@callBack')->name('callBack');
    ...
    Route::get('checkout-details', 'WebController@checkout')->name('checkout')->middleware('customer');
    Route::post('checkout-payment', 'WebController@checkout_payment')->name('checkout-payment')->middleware('customer');
...
//    Route::get('checkout-review', 'WebController@checkout_review')->name('checkout-review')->middleware('customer');
    Route::get('get_payment_link', 'WebController@getSession')->name('getPaymentLink')->middleware('customer');
    Route::post('checkout-complete', 'WebController@checkout_complete')->name('checkout-complete')->middleware('customer');
    ...
    Route::get('terms-and-conditions', 'WebController@termsandCondition')->name('terms-and-conditions');
    Route::get('privacy-policy', 'WebController@privacy_policy')->name('privacy-policy');
    Route::get('pull', 'WebController@pull')->name('pull');
    Route::get('shipping-and-exchange', 'WebController@shipping_and_exchange')->name('shipping-and-exchange');
    Route::get('faqs', 'WebController@faq')->name('faq');
...
//    Route::get('products', 'WebController@products')->name('products');
    ...
    Route::get('store-wishlist/{productSlug}', 'WebController@storeWishlist')->name('store-wishlist');
    Route::get('delete-wishlist/{productSlug}', 'WebController@deleteWishlist')->name('delete-wishlist');
    Route::get('delete-all-wishlist/', 'WebController@deleteAllWishlist')->name('delete-all-wishlist');

    Route::post('/currency', 'CurrencyController@changeCurrency')->name('currency.change');
    Route::get('/request-an-item/{productId?}', 'WebController@requestAnItem')->name('request');
    Route::post('/request-an-item', 'WebController@requestAnItemSubmit')->name('request');

    Route::get('/consign', 'WebController@consign')->name('consign');
    Route::post('/consign', 'WebController@consignSubmit')->name('consign');

    Route::get('about-us', 'WebController@about_us')->name('about-us');

    Route::get('/look-books', 'WebController@look_books')->name('look-books');
    Route::get('/look-book/{slug}', 'WebController@look_book_gallery')->name('look_book_gallery');
    Route::get('/how-we-authenticate', 'WebController@how_we_authenticate')->name('how-we-authenticate');
    ...
    Route::get('account-order', 'UserProfileController@account_order')->name('account-order');
    Route::get('account-order-details/{order_id}', 'UserProfileController@account_order_details')->name('account-order-details');
    ...
    Route::get('account-security', 'UserProfileController@account_security')->name('account-security');
    Route::post('account-security', 'UserProfileController@change_password')->name('account-security');
    ...
    Route::post('contact-us', 'WebController@contact_us_store')->name('contact-us');

    // Shop
    Route::get('shop/{category?}/{sub_category?}', 'WebController@shop')->name('shop');
    Route::get('product/{slug?}', 'WebController@productDetails')->name('product.details');
    //sellerShop
...
//    Route::post('variant_price', 'CartController@variant_price')->name('variant_price');
    ...
    Route::get('list', 'CartController@index')->name('list');
    Route::get('remove/{key}', 'CartController@removeFromCart')->name('remove');
    Route::get('remove/all', 'CartController@removeAllFromCart')->name('remove.all');
