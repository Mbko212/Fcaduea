    ...
    public function user_account(Request $request)
    {
        if (auth('customer')->check()) {
            $orders = Order::with(['details'])
                ->where('customer_id',auth('customer')->id())->latest()->get();
            $return_orders = Order::with(['details'])
            ->where('customer_id',auth('customer')->id())
            ->where('order_status','returned')->latest()->get();
            $cancelled_orders = Order::with(['details'])
                ->where('customer_id',auth('customer')->id())
                ->where('order_status','canceled')->latest()->get();
//            return $orders;
            $customerDetail = User::where('id', auth('customer')->id())->first();
            return view('front.profile', compact('customerDetail','orders','return_orders','cancelled_orders'));
        } else {
