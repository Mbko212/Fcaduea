...
//        return $categories;
    ...
    public function checkout(Request $request)
    {
        ...
        if (session()->has('cart') && count(session('cart')) > 0) {
            return view('front.shipment', compact('shipping_methods', 'shipping_addresses','customerDetail','cart'));
        }
...
//    }
    ...
    {
        try {
            if(empty($request->success) || $request->success != "True"){
                Toastr::error("payment did not succeed!");
                return redirect()->route('home');
            }
            if(!empty($request->reference)){
                $order = Order::where('transaction_ref',$request->reference)->first();
                ...
                Toastr::info("ThankYou For Your Order!");
                Mail::to(auth('customer')->user()->email)->send(new \App\Mail\OrderPlaced($order->id));
...
//        }
        ...
        $discount = session()->has('coupon_discount') ? session('coupon_discount') : 0;
            ...
            $unq_ref_no = md5(microtime().rand());
            $string = auth('customer')->user()->email.auth('customer')->id()."test"."12345678"."test"."zT5685Am4".$unq_ref_no;
        ...
        foreach (session('cart') as $c) {
            ...
            $getPaymentLink = $this->getSession($unq_ref_no,$string,$order_amount,$order_id);
//            return $getPaymentLink;
            if ($getPaymentLink['success']) {
                Order::where('id', $order_id)->update(['transaction_ref' => $getPaymentLink['Reference']]);
                return redirect($getPaymentLink['PaymentUrl']);
            } else {
                Toastr::error($getPaymentLink['message']);
                return back();
            }
...
//
        $fetched = Product::active()->paginate(12);
        ...
        if($search_category || $sub_category){
            ...
            foreach ($products as $product) {
                ...
                if($search_category && $sub_category){
                    if(!empty($productCategory[0]) && !empty($productCategory[1]) &&
                        $productCategory[0]['slug'] == $search_category &&
                        $productCategory[1]['slug'] == $sub_category){
...
//    }
    ...
    public function contact_us_store(Request $request)
    {
        ...
        Toastr::info('Your message send successfully!');
        return back();
    }
...
//        return $terms_and_conditions;
        return view('front.termsConditions', compact('terms_and_conditions'));
    }
    ...
    public function faq()
    {
        $help_topics = HelpTopic::Status()->get();
    ...
    public function newsletter(Request $request)
    {
        try {
            ...
            if(!empty($check)){
                Toastr::warning('Already Join Newsletter!!');
    ...
    public function get_access_token() {
        ...
        $ch = curl_init("http://151.106.28.182:9222/token");
        // $ch = curl_init("https://noqoodypay.com/sdk/token");
    ...
    public function getSession($unq_ref_no, $string,$amount,$order_no) {
        $access_token = $this->get_access_token();
        $string = "test@test.com"."test"."12345678"."test"."zT5685Am4".$unq_ref_no;
        $sig = base64_encode(hash_hmac('sha256', $string, '4D6251655468576D', true));
        $fields_string = '';
        $args = array(
            'ProjectCode'    => "zT5685Am4",
            'Description'    => "test",
        ...
        // Performing the HTTP request
        $ch = curl_init("http://151.106.28.182:9222/api/PaymentLink/GenerateLinks");
        // $ch = curl_init("https://noqoodypay.com/sdk/api/PaymentLink/GenerateLinks");
        curl_setopt($ch, CURLOPT_HTTPHEADER, $request_headers);
        curl_setopt($ch,CURLOPT_POST, count($args));
        curl_setopt($ch,CURLOPT_POSTFIELDS, $fields_string);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
//3c607a768ebb652a2934d49eace09646
//3c607a768ebb652a2934d49eace096
